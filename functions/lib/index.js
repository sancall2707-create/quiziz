...
// ============================================================
exports.setStudentClaimOnCreate = (0, identity_1.beforeUserCreated)(async (event) => {
    ...
    try {
        ...
        switch (data.type) {
            ...
            case 'challenge_bonus': {
                const p = data.payload;
                const stars = clampInt(p.stars, 0, missionRewards_1.CHALLENGE_LIMITS.maxStars, 0);
                const xp = clampInt(p.xp, 0, missionRewards_1.CHALLENGE_LIMITS.maxXp, 0);
                const coins = clampInt(p.coins, 0, missionRewards_1.CHALLENGE_LIMITS.maxCoins, 0);
                const result = await firestore.runTransaction(async (tx) => {
                    const snap = await tx.get(userRef);
                    if (!snap.exists)
                        throw new https_1.HttpsError('not-found', 'Profil pengguna tidak ditemukan.');
