...
const CURRENT_USER_KEY = 'codenusa_current_user_id';
...
const INITIAL_ASSIGNMENTS: Assignment[] = [
  {
    ...
    description: 'Selesaikan eksplorasi perangkat keras dan pelajari fungsi mouse & keyboard bersama Robot Kobi.',
    targetGrade: 4,
    type: 'mission',
    dueDate: '2026-09-05',
    createdAt: new Date().toISOString(),
    createdBy: {
      uid: 'user-teacher-wijaya',
      name: 'Wijaya',
      role: 'teacher',
      username: 'wijaya_guru'
    },
    missionId: 'm-g4-c1-m4'
  }
];
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  // 1. Initial State Loaders from localStorage with fail-safe fallbacks
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const filtered = parsed.filter(u => !DEMO_USER_IDS.includes(u.id));
          const hasAdmin = filtered.some(u => u.id === 'user-admin-wijaya');
          const hasTeacher = filtered.some(u => u.id === 'user-teacher-wijaya');
          const finalUsers = [...filtered];
          if (!hasAdmin) {
            const admin = INITIAL_SYSTEM_USERS.find(u => u.id === 'user-admin-wijaya');
            if (admin) finalUsers.push(admin);
          }
          if (!hasTeacher) {
            const teacher = INITIAL_SYSTEM_USERS.find(u => u.id === 'user-teacher-wijaya');
            if (teacher) finalUsers.push(teacher);
          }
          return finalUsers;
        }
      }
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    try {
      ...
      if (session && session.userId && !DEMO_USER_IDS.includes(session.userId)) {
        return true;
      }
      return false;
    } catch {
  ...
  const [mustChangePassword, setMustChangePassword] = useState<boolean>(() => {
    try {
      ...
      if (session && session.userId) {
        return isPasswordChangeRequired(session.userId);
      }
    } catch {
      // Fallback
    }
  ...
  const [isSimulatedOffline, setIsSimulatedOffline] = useState<boolean>(() => {
    try {
      return localStorage.getItem('codenusa_simulated_offline') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const [storageEstimate, setStorageEstimate] = useState<
    { usedBytes: number; quotaBytes: number; percentage: number } | undefined
  >(undefined);

  // Cheers / Apresiasi Papan Skor
  const [cheersMap, setCheersMap] = useState<Record<string, number>>(() => {
    try {
      const saved = localStorage.getItem(CHEERS_KEY);
  ...
  // Test Firestore Connection and setup realtime listeners
  useEffect(() => {
    ...
    const unsubscribeUsers = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          ...
          if (u && u.id && !DEMO_USER_IDS.includes(u.id)) {
            firestoreUsers.push(u);
          }
        });

        // Merge with initial system users (Wijaya Admin & Teacher)
        const combined = [...firestoreUsers];
        INITIAL_SYSTEM_USERS.forEach((sysUser) => {
          if (!combined.some((u) => u.id === sysUser.id || u.username === sysUser.username)) {
    ...
    // 2. Listen to 'assignments' collection in real-time
    ...
    // Seed default system users and initial assignment to Firestore if collection is empty
  ...
  const cheerStudent = useCallback((studentId: string) => {
    setCheersMap(prev => {
      ...
      try {
        localStorage.setItem(CHEERS_KEY, JSON.stringify(updated));
      } catch (e) {
        console.warn('Cheers sync error:', e);
      }
      return updated;
    });
  ...
  // Compute reactive combined leaderboard students
  const allLeaderboardStudents = useMemo(() => {
    ...
    for (const globalPeer of GLOBAL_LEADERBOARD_STUDENTS) {
      if (!registeredIds.has(globalPeer.id)) {
        mergedList.push({
          ...globalPeer,
          cheerCount: (globalPeer.cheerCount || 0) + (cheersMap[globalPeer.id] || 0),
          isCurrentUser: globalPeer.id === currentUserId
        });
      }
    }
  ...
  useEffect(() => {
    registerServiceWorker().then(registered => {
      setServiceWorkerActive(registered || isServiceWorkerActive());
    });
    ...
    const handleOnline = () => {
      ...
      audioService.playCoinSound();
      triggerKobiSpeech(
        'Internet terhubung kembali! Sistem siap menyinkronkan data petualanganmu.',
        'happy',
        true
      );
    };
    ...
    const handleOffline = () => {
      setIsOnline(false);
      triggerKobiSpeech(
        'Koneksi internet terputus. Mode offline aktif! Progres misimu tetap aman tersimpan.',
        'helping',
        true
      );
    };
  ...
  const persistProjects = useCallback((projs: CodingProject[]) => {
    try {
      ...
      console.warn('LocalStorage projects sync error:', e);
    }
  }, []);

  const persistDailyMissions = useCallback((dMissions: DailyMission[]) => {
    try {
      localStorage.setItem(DAILY_MISSIONS_KEY, JSON.stringify(dMissions));
    } catch (e) {
      console.warn('LocalStorage daily missions sync error:', e);
    }
  }, []);
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      ...
      } else if (e.key === CURRENT_USER_KEY && e.newValue) {
        setCurrentUserId(e.newValue);
      } else if (e.key === ACTIVE_MISSION_KEY && e.newValue) {
        setActiveMissionIdState(e.newValue);
      } else if (e.key === PROJECTS_KEY && e.newValue) {
        try {
          const parsed = JSON.parse(e.newValue);
          if (Array.isArray(parsed)) {
            setProjects(parsed);
      ...
      } else if (e.key === DAILY_MISSIONS_KEY && e.newValue) {
        try {
          const parsed = JSON.parse(e.newValue);
          if (Array.isArray(parsed)) {
  ...
  useEffect(() => {
    persistDailyMissions(dailyMissions);
  }, [dailyMissions, persistDailyMissions]);
  ...
  const recordDailyActivity = useCallback((reason: string = 'Aktivitas Harian') => {
    ...
    setAllUsers(prev => prev.map(u => {
      if (u.id === currentUserId) {
        ...
        if (newStreak >= 3 && !newBadges.includes('badge-streak-3')) {
          newBadges.push('badge-streak-3');
          bonusCoins += 50;
          bonusXp += 100;
        }
        if (newStreak >= 7 && !newBadges.includes('badge-streak-7')) {
          newBadges.push('badge-streak-7');
          bonusCoins += 150;
          bonusXp += 250;
        }
        if (newStreak >= 14 && !newBadges.includes('badge-streak-14')) {
          newBadges.push('badge-streak-14');
          bonusCoins += 300;
          bonusXp += 500;
        }
  ...
  const checkInStreak = () => {
    ...
    if (isAlreadyToday) {
      ...
      return {
        streakCount: currentUser.streakDays,
        isFirstToday: false,
        bonusXp: 0,
        bonusCoins: 0
      };
    ...
    if (newStreak >= 3 && !currentUser.badges.includes('badge-streak-3')) {
      milestoneBadge = BADGES_DATA.find(b => b.id === 'badge-streak-3');
    } else if (newStreak >= 7 && !currentUser.badges.includes('badge-streak-7')) {
      milestoneBadge = BADGES_DATA.find(b => b.id === 'badge-streak-7');
    } else if (newStreak >= 14 && !currentUser.badges.includes('badge-streak-14')) {
      milestoneBadge = BADGES_DATA.find(b => b.id === 'badge-streak-14');
    }

    setAllUsers(prev => prev.map(u => {
      if (u.id === currentUserId) {
        const newBadges = milestoneBadge && !u.badges.includes(milestoneBadge.id)
          ? [...u.badges, milestoneBadge.id]
          : u.badges;
        ...
        const updatedUser: User = {
          ...
          lastActive: 'Hari ini',
          lastActiveDate: todayStr,
          xp: u.xp + bonusXp + (milestoneBadge ? 100 : 0),
          coins: u.coins + bonusCoins + (milestoneBadge ? 50 : 0),
          badges: newBadges
        };
  ...
  useEffect(() => {
    if (currentUser?.settings) {
      audioService.setSettings(
        ...
        currentUser.settings.narrationVoiceEnabled
      );
    }
  ...
  const login = async (
    ...
    const res = await validateCredentials(identifier, passwordInput, allUsers);
    if (res.valid && res.user) {
      ...
      try {
        localStorage.setItem(CURRENT_USER_KEY, res.user.id);
      } catch {
        // ignore
      }
      triggerKobiSpeech(`Selamat datang kembali, ${res.user.name}!`, 'happy', true);
  ...
  const updatePassword = async (newPassword: string): Promise<{ success: boolean; message?: string }> => {
    const res = await changeUserPassword(currentUserId, newPassword);
    if (res.success) {
  ...
  const registerStudent = async (
    ...
    if (res.success && res.user) {
      ...
      triggerKobiSpeech(
        `Selamat datang di CodeNusa, ${data.nickname || data.name}! Ayo mulai petualangan kodingmu!`,
        'celebrating',
        true
      );
    }
  ...
  const logout = () => {
    ...
    triggerKobiSpeech(
      'Sampai jumpa lagi! Jangan lupa kembali besok untuk menjaga streak belajarmu.',
      'waving',
      false
    );
  };
  ...
  const switchUser = (userId: string) => {
    ...
    if (targetUser) {
      triggerKobiSpeech(`Selamat datang kembali, ${targetUser.name}!`, 'happy', true);
    }
  };
  ...
  const setKobiPosition = (nodeId: string, missionId?: string) => {
    ...
    setAllUsers(prev => {
      const updated = prev.map(u => {
        if (u.id === currentUserId) {
          const userUpdated = { ...u, kobiPosition: nodeId };
          try {
            const userRef = doc(db, 'users', u.id);
            setDoc(userRef, { kobiPosition: nodeId }, { merge: true });
          } catch {
            // ignore
          }
          return userUpdated;
  ...
  const updateUserGrade = (grade: ClassGrade) => {
    setAllUsers(prev => {
      const updated = prev.map(u => {
        if (u.id === currentUserId) {
          const userUpdated = { ...u, grade };
          try {
            const userRef = doc(db, 'users', u.id);
            setDoc(userRef, { grade }, { merge: true });
          } catch {
            // ignore
          }
          return userUpdated;
    ...
    setAllUsers(prev => {
      const updated = prev.map(u => {
        if (u.id === currentUserId) {
          const userUpdated = {
            ...u,
            settings: { ...u.settings, ...newSettings }
          };
          try {
            const userRef = doc(db, 'users', u.id);
            setDoc(userRef, { settings: userUpdated.settings }, { merge: true });
          } catch {
            // ignore
          }
          return userUpdated;
  ...
  const toggleSimulatedOffline = (simulate?: boolean) => {
    setIsSimulatedOffline(prev => {
      ...
      try {
        localStorage.setItem('codenusa_simulated_offline', String(nextVal));
      } catch {
        // ignore
      }
      audioService.playSnapSound();
      if (nextVal) {
        triggerKobiSpeech(
          'Simulasi Mode Offline diaktifkan! Kamu tetap bisa belajar dan bermain tanpa internet.',
          'helping',
          true
        );
      } else {
        triggerKobiSpeech('Kembali ke Mode Online! Menghubungkan ke server...', 'happy', true);
        setTimeout(() => {
          syncOfflineData();
        }, 600);
      }
  ...
  const syncOfflineData = async (): Promise<{ syncedCount: number }> => {
    ...
    audioService.playFanfare();
    triggerKobiSpeech(
      `Hore! Sebanyak ${syncedCount} catatan progres misi offline berhasil disinkronkan ke server!`,
      'celebrating',
      true
    );
  ...
  const completeMission = (missionId: string, stars: number, score: number) => {
    ...
    for (const ch of CHAPTERS_DATA) {
      ...
      if (idx !== -1) {
        missionObj = ch.missions[idx];
        missionIndex = idx;
        if (idx + 1 < ch.missions.length) {
          nextMissionObj = ch.missions[idx + 1];
        }
        break;
      }
    }
    ...
    if (!isAlreadyClaimed && missionObj) {
      ...
      if (missionObj.rewardBadgeId) {
        const b = BADGES_DATA.find(bg => bg.id === missionObj?.rewardBadgeId);
        if (b && !currentUser.badges.includes(b.id)) {
          newBadge = b;
        }
      }
    }
    ...
    if (nextMissionObj) {
      setActiveMissionId(nextMissionObj.id);
    }
    ...
    setAllUsers(prev => {
      const updated = prev.map(u => {
        if (u.id === currentUserId) {
          ...
          const updatedBadges = newBadge && !u.badges.includes(newBadge.id)
            ? [...u.badges, newBadge.id]
            : u.badges;
          ...
          const userUpdated: User = {
            ...
            xp: newXp,
            level: newLevel,
            stars: u.stars + starsEarned,
            coins: u.coins + coinsEarned,
            completedMissions: completed,
            rewardsClaimed: updatedClaimed,
            kobiPosition: newKobiNode,
            badges: updatedBadges,
            missionScores: {
              ...u.missionScores,
              [missionId]: {
                stars: Math.max(stars, u.missionScores?.[missionId]?.stars || 0),
                score: Math.max(score, u.missionScores?.[missionId]?.score || 0),
          ...
          try {
            const userRef = doc(db, 'users', u.id);
            setDoc(userRef, userUpdated, { merge: true });
          } catch {
            // ignore
          }
    ...
    if (isOffline) {
      isOfflineSaved = true;
      const offlineRecord = saveToOfflineQueue({
        type: 'mission_complete',
        userId: currentUserId,
        title: missionObj?.title || 'Misi Pembelajaran',
        subtitle: `${stars} Bintang • Skor ${score}`,
        payload: {
          missionId,
          stars,
          score,
      ...
      getStorageEstimate().then(setStorageEstimate);
      triggerKobiSpeech(
        'Misi berhasil diselesaikan! Karena koneksi internet tidak stabil, progres disimpan sementara secara lokal di perangkat ini.',
        'happy',
        false
      );
    }
  ...
  const saveProject = (project: CodingProject) => {
    setProjects(prev => {
      ...
      if (idx >= 0) {
        const copy = [...prev];
        copy[idx] = { ...project, updatedAt: new Date().toISOString().split('T')[0] };
        updated = copy;
      } else {
    ...
    if (isOffline) {
      const offRec = saveToOfflineQueue({
        type: 'coding_project',
        userId: currentUserId,
        title: `Proyek: ${project.title}`,
        subtitle: `${project.blocks.length} Blok Kode Tersimpan`,
        payload: {
    ...
    triggerKobiSpeech(
      isOffline
        ? 'Proyek kodemu disimpan di memori lokal offline!'
        : 'Proyek kodemu berhasil disimpan!',
      'happy',
      true
    );
        ...
        if (dm.id === missionId && !dm.isCompleted) {
          ...
          setAllUsers(uList => {
            const updatedUsers = uList.map(u => {
              if (u.id === currentUserId) {
                const userUpdated: User = {
                  ...u,
                  stars: u.stars + dm.rewardStars,
                  coins: u.coins + dm.rewardCoins
                };
                try {
                  const userRef = doc(db, 'users', u.id);
                  setDoc(userRef, { stars: userUpdated.stars, coins: userUpdated.coins }, { merge: true });
                } catch {
                  // ignore
                }
                return userUpdated;
          ...
          if (isOffline) {
            const offRec = saveToOfflineQueue({
              type: 'daily_claim',
              userId: currentUserId,
              title: `Misi Harian: ${dm.title}`,
              subtitle: `+${dm.rewardStars} Bintang • +${dm.rewardCoins} Koin`,
              payload: {
                missionId: dm.id,
                stars: dm.rewardStars,
                coinsEarned: dm.rewardCoins,
                timestamp: new Date().toISOString()
              }
            });
  ...
  const createAssignment = async (
    data: Omit<Assignment, 'id' | 'createdAt' | 'createdBy'>
    ...
    try {
      ...
      // Save to Firestore
      try {
        const asgRef = doc(db, 'assignments', newId);
        await setDoc(asgRef, newAssignment);
      } catch (err) {
      ...
      return { success: false, message: 'Gagal membuat tugas. Silakan coba lagi.' };
    }
  };
  ...
  const updateUserProfile = async (
    ...
    try {
      ...
      // Security check: User can only edit own profile unless admin
      if (currentUser.role !== 'admin' && currentUser.id !== userId) {
        return { success: false, message: 'Kamu hanya dapat mengubah profil milikmu sendiri.' };
      }

      // Teacher cannot edit Admin
      if (currentUser.role === 'teacher' && targetUser.role === 'admin' && currentUser.id !== userId) {
        return { success: false, message: 'Guru tidak diizinkan mengubah profil admin.' };
      }

      // Student cannot edit others
      if (currentUser.role === 'student' && currentUser.id !== userId) {
        return { success: false, message: 'Siswa hanya dapat mengubah profil sendiri.' };
      }
      ...
      // 4. Update React State immediately (no refresh needed)
      setAllUsers(prev => {
        const updated = prev.map(u => {
          if (u.id === userId) {
            return {
      ...
      // 5. Update local auth registry if matching
      try {
        const registry = getAuthRegistry();
        if (registry[userId]) {
          registry[userId].nickname = cleanNickname;
          localStorage.setItem('codenusa_auth_registry_v1', JSON.stringify(registry));
        }
      } catch {
        // ignore
      }
  ...
  const triggerKobiSpeech = (text: string, mood: KobiSpeechState['mood'] = 'happy', speakAudio: boolean = false) => {
    ...
    if (speakAudio && currentUser?.settings.narrationVoiceEnabled) {
      audioService.speakText(text);
    }
  };

  const dismissKobiSpeech = () => {
    setKobiSpeech(null);
  ...
  const awardChallengeBonus = (stars: number, xp: number, challengeTitle: string) => {
      ...
      const updated = prev.map(u => {
        if (u.id === currentUserId) {
          ...
          const userUpdated: User = {
            ...
            level: newLevel,
            stars: u.stars + stars,
            coins: u.coins + (stars * 10)
          };
          try {
            const userRef = doc(db, 'users', u.id);
            setDoc(userRef, { xp: userUpdated.xp, level: userUpdated.level, stars: userUpdated.stars, coins: userUpdated.coins }, { merge: true });
          } catch {
            // ignore
          }
          return userUpdated;
    ...
    if (isOffline) {
      const offRec = saveToOfflineQueue({
        ...
        userId: currentUserId,
        title: `Tantangan Algoritma: ${challengeTitle}`,
        subtitle: `+${stars} Bintang • +${xp} XP`,
        payload: {
  ...
  const resetProgress = () => {
    setAllUsers(INITIAL_SYSTEM_USERS);
    persistUsers(INITIAL_SYSTEM_USERS);
    setDailyMissions(DAILY_MISSIONS);
    persistDailyMissions(DAILY_MISSIONS);
    clearOfflineQueue();
    setOfflineQueue([]);
    audioService.playSuccessSound();
    triggerKobiSpeech('Data progres berhasil di-reset ke data awal!', 'happy', true);
  };
